...
int main(int argc, char** argv) {
	...
	for (int i = 0; i < number_of_retransmissions; ++i) {
		zmq::message_t message(transform_data.size());
		std::memcpy(message.data(), transform_data.data(), transform_data.size());
		std::cout << "Sending message with size " << transform_data.size() << " using [" << ss_bind.str() << "]: " << transform_data << std::endl;
